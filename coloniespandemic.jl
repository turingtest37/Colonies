...
@enum Health atrisk infected healed dead
...
function initialize_colony(xsize::Integer, ysize::Integer, seed::ColonySeed)
    P = fill(0::Int64,(4))
    c = Array{Array{Int64,1},2}(ysize,xsize)
    if seed==random
        # random
        c = [rand(1:500,(4)) for j=1:ysize, i=1:xsize]
    else
        # square or blank
        P[1] = 1000
        c = fill(P,(ysize,xsize))
    end
...
function drawrepeatidx(cctx::CairoContext, repeatidx::Int64, startpt:ccolor:Tuple{Int64,Int64}; color=(1,0,0))
    Cairo.save(cctx)
...
function draw_colony(colony::Array{Array{Int64,1},2}, cctx::CairoContext)
    draw_colony(colony, 0, false, cctx)
end

function draw_colony(colony::Array{Array{Int64,1},2}, repeatidx::Int64, drawrepeat::Bool, cctx::CairoContext)
    # println("Drawing colony $(size(colony))")
    co_x=0
    co_y=0
    for co_y in indices(colony,1)
      for co_x in indices(colony,2)
          val::Array{Int64,1} = colony[co_y,co_x]
...
function counthealth(v::Health, val::Health)
    v==val ? 1 :0
end

# Count neighbors+self in each state --> P vector
# choices:
# 1) S -> I (i) : Use count of infected neighbors as n in hn^n to determine local probability of infection
# 2) S -> V (v): Count all S in current generation and use to determine prob. of vaccination according to max capacity
# 3) S -> M  (m): natural death rate
# 4) S -> S : 1-(sum(i,v,m))
# 5) I -> M (d) : seasonally variable death rate from infection
# 6) Repopulation : M -> S || M -> I || M -> R - count neighbors; flip state to that of greatest neighbor population; in case of tie, S > R > I
# new state = P * TM => find column of max(new P) => new state for i,j
# every square has N people, arrayed in P according to their health [SIVM]
# on each turn, recalculate T based on the neighbors:
function calcreducedval(neighborhood::Array{Array{Int64,1},2}, mask_slice::SubArray)
    # code to extract first element of the array at i,j
    indx = indices(neighborhood)
    Sweighted = neighborhood[1,1])[1]
    Iweighted = sum([(mask_slice[i,j] * neighborhood[i,j])[2] for i=indx[1],j=indx[2]])
    Vweighted =
    Mweighted =

    n = sum(counthealth.(neighborhood,infected))

end

function calculatenewcolony(colony::Array{Array{Float64},2}, context::CommonwealthContext)
    ...
    new_colony::Array{Array{Float64},2} = initialize_colony(colonyxsize, colonyysize, blank)
    ...
    for co_y::Int64 in indices(colony,1)
        for co_x::Int64 in indices(colony,2)
            ...
            neighborhood::Array{Array{Float64},2} = [i[1] for i in view(colony, colony_slice_y, colony_slice_x)]
            # println("neighborhood=$neighborhood")
            reduced_val::Array{Int64} = calcreducedval(neighborhood,rawmask,mask_slice)
            ...
            # end
            new_colony[co_y,co_x] = reduced_val
        end
    end
    new_colony
end
